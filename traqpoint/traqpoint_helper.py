...
class traqpoint_helper(nn.Module):
    ...
    @torch.inference_mode()
    def match_pose(self, img0, img1, thr=0.01, resize=None, top_k=4096):
        if top_k is not None and top_k != self.TraqPoint.top_k:
            self.TraqPoint.top_k = top_k
            self.TraqPoint.set_softdetect(top_k=top_k)
        ...
        out0 = self.TraqPoint.extract_pose(img0)[0]
        out1 = self.TraqPoint.extract_pose(img1)[0]
    ...
    def match_lg(self, img0, img1, thr=0.01, resize=None, top_k=4096):
        if self.lg_matcher is None:
            lg_conf = {
                ...
                "weights": './weights/traqpoint_lg-v2.pth',  # path to the weights
            }
            self.lg_matcher = LightGlue('traqpoint', **lg_conf).to(self.TraqPoint.device)
    ...
    def match_dense(self, img0, img1, thr=0.01, resize=None, anchor='mnn'):
        ...
        out0 = self.TraqPoint.extract_dense(img0)[0]
        out1 = self.TraqPoint.extract_dense(img1)[0]
        
        # get top_k confident matches
        mkpts0, mkpts1, conf = self.dense_matcher(out0, out1, thr, err_thr=self.TraqPoint.stride, anchor=anchor)
    ...
    def match_3rd_party(self, img0, img1, model='aliked', resize=None, thr=0.01):
        ...
        out0 = self.TraqPoint.extract_3rd_party(img0, model=model)[0]
        out1 = self.TraqPoint.extract_3rd_party(img1, model=model)[0]
    ...
    def parse_input(self, x, resize=None):
        ...
        scale = torch.Tensor([x.shape[-1] / w, x.shape[-2] / h]).to(self.TraqPoint.device)
