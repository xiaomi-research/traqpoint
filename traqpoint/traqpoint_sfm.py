...
class TraqPoint(nn.Module):
    ...
    def _print_score_distribution(self, scores: torch.Tensor, prefix: str = "kptscores", bins: int = 20):
        """
        Print score distribution stats to terminal:
        - count/min/max/mean/std
        - percentiles
        - histogram
        - per-batch counts above 0.25 (if scores is [B, K])
        """
        with torch.inference_mode():
            if not torch.is_tensor(scores):
                scores = torch.tensor(scores)
            s = scores.detach().float().reshape(-1)
            s_cpu = s.to('cpu').numpy()

            mn, mx = float(s.min()), float(s.max())
            mean, std = float(s.mean()), float(s.std())
            pcts_q = [0, 1, 5, 10, 25, 50, 75, 90, 95, 99, 100]
            pcts = np.percentile(s_cpu, pcts_q)
            hist, edges = np.histogram(s_cpu, bins=bins)

            print(f"[TraqPoint] {prefix} count={s_cpu.size} min={mn:.6f} max={mx:.6f} mean={mean:.6f} std={std:.6f}")
            print("[TraqPoint] " + prefix + " percentiles: " + " ".join([f"P{q}={v:.6f}" for q, v in zip(pcts_q, pcts)]))
            print(f"[TraqPoint] {prefix} histogram (bins={bins}):")
            for i in range(len(hist)):
                print(f"  [{edges[i]:.6f}, {edges[i+1]:.6f}): {int(hist[i])}")

            # Per-batch threshold counts if shape is [B, K]
            if scores.dim() == 2:
                thr = 0.25
                cnts = (scores > thr).sum(dim=1).tolist()
                total = scores.shape[1]
                print(f"[TraqPoint] {prefix} per-batch > {thr}: " + ", ".join([f"{int(c)}/{total}" for c in cnts]))
    ...
    def extract(self, x):
        ...
        # self._print_score_distribution(kptscores, prefix="kptscores (after SoftDetect)", bins=30)
        ...
        valid = kptscores > 0.3 #self.detection_threshold
...
def build(config=None, weights=None):
    ...
    config['weights'] = "./weights/traqpoint_best.pth"
