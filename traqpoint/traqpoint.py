...
class TraqPoint(nn.Module):

    def __init__(self, detector, descriptor, detection_threshold=0, top_k=4096, train_detector=False, device='cuda'):
        super().__init__()
    ...
    def forward(self, samples: NestedTensor):
        ...
        policy_map, log_policy_map, raw_logits  = self.detector(samples)
        
        feats, matchibility = self.descriptor(samples)
        
        return feats,  matchibility,  policy_map, log_policy_map, raw_logits
    ...
    def extract(self, x):
        ...
        B, _, _H1, _W1 = x.shape
        M1, H1,  K1, K_log, raw_logits  = self.forward(x)
        M1 = F.normalize(M1, dim=1)
        # pose estimate
        keypoints, kptscores, scoredispersitys = self.softdetect(raw_logits)
        ...
        # Threshold by configured detection_threshold (default 0.0 to match prior behavior)
        if not hasattr(self, "_printed_det_th"):
            try:
                print(f"[TraqPoint.extract] detection_threshold={self.detection_threshold}")
            except Exception:
                pass
            self._printed_det_th = True
    ...
    def extract_pose(self, x):
        ...
        # Use RDD-style detector head for pose-only inference.
        # This does not change the default detector.forward() behavior used elsewhere.
        if not isinstance(x, NestedTensor):
            x_nt = nested_tensor_from_tensor_list(x)
        else:
            x_nt = x
        K1_pose, K_log_pose, raw_logits_pose = self.detector.forward_with_raw(x_nt)

        feats, matchibility = self.descriptor(x_nt)
        M1 = F.normalize(feats, dim=1)

        # pose estimate
        keypoints, kptscores, scoredispersitys = self.softdetect(K1_pose)
        ...
        keypoints = keypoints * torch.tensor([rw1,rh1], device=keypoints.device).view(1, -1)
        # Threshold by configured detection_threshold (default 0.0 to match prior behavior)
        if not hasattr(self, "_printed_det_th_pose"):
            try:
                print(f"[TraqPoint.extract_pose] detection_threshold={self.detection_threshold}")
            except Exception:
                pass
            self._printed_det_th_pose = True
        valid = kptscores > self.detection_threshold
    ...
    @torch.inference_mode()
    def extract_vo_v2(self, image: np.ndarray):
        
        self.set_softdetect(top_k=self.top_k)
        x, _ = self.parse_input(image, None)
        ...
        if not hasattr(self, "_printed_det_th"):
            try:
                print(f"[TraqPoint.extract_vo_v2] detection_threshold={self.detection_threshold}")
            except Exception:
                pass
            self._printed_det_th = True
        valid = kptscores > self.detection_threshold
    ...
    def parse_input(self, x, resize=None):
        ...
        scale = torch.Tensor([x.shape[-1] / w, x.shape[-2] / h]).to(self.device)

        return x, scale

    def get_new_image_size(self, H, W, resize = 1024):
        if isinstance(resize, (tuple, list)):
            H_t, W_t = resize
            H_new, W_new = H_t, W_t
        else:
            H_new, W_new = resize, resize
        scale = min(W_new / float(W), H_new / float(H))
        W_new, H_new = int(round(W * scale)), int(round(H * scale))
        H_new = int(round(H_new / 32.0) * 32)
        W_new = int(round(W_new / 32.0) * 32)
        return H_new, W_new
    @torch.inference_mode()
...
def build(config=None, weights=None):
    ...
    if 'weights' in config and config['weights'] is not None:
        model.load_state_dict(torch.load(config['weights'], map_location='cpu'), strict=False)
