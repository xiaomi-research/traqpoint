...
from benchmarks.utils import pose_auc, compute_symmetrical_epipolar_errors, compute_pose_error, compute_relative_pose, estimate_pose, dynamic_alpha
...
class ScanNetPoseMNNBenchmark:
    def __init__(self, data_root="./scannet_test_1500") -> None:
        self.scenes = np.load(f'{data_root}/test.npz', allow_pickle=True)['name']
    ...
    def benchmark(self, model_helper, model_name = None, scale_intrinsics = False, calibrated = True, plot_every_iter=1, plot=False, method='sparse'):
        ...
        with torch.no_grad():
            ...
            for pairind in tqdm(range(len(self.scenes))):

                scene_name, scene_sub_name, stem_name_0, stem_name_1 = self.scenes[pairind]
                
                seq_name = f'scene{scene_name:04d}_{scene_sub_name:02d}'
                
                K0 = K1 = self.intrinsics[seq_name].copy()
                
                T0 = read_scannet_pose(f"{data_root}/scene{scene_name:04d}_{scene_sub_name:02d}/pose/{stem_name_0}.txt")
                T1 = read_scannet_pose(f"{data_root}/scene{scene_name:04d}_{scene_sub_name:02d}/pose/{stem_name_1}.txt")
                R0, t0 = T0[:3, :3], T0[:3, 3]
                R1, t1 = T1[:3, :3], T1[:3, 3]
                R, t = compute_relative_pose(R0, t0, R1, t1)
                T0_to_1 = np.concatenate((R,t[:,None]), axis=-1)
                
                im_A_path = f"{data_root}/scene{scene_name:04d}_{scene_sub_name:02d}/color/{stem_name_0}.jpg"
                im_B_path = f"{data_root}/scene{scene_name:04d}_{scene_sub_name:02d}/color/{stem_name_1}.jpg"
                ...
                if method == 'dense':
                    ...
                    kpts0, kpts1, conf = model_helper.match_lg(im_A, im_B, thr=0.01, resize=None)
                elif method == 'sparse':
                    kpts0, kpts1, conf = model_helper.match_pose(im_A, im_B, thr=0.01, resize=None)
                else:
                    kpts0, kpts1, conf = model_helper.match_3rd_party(im_A, im_B, thr=0.01, resize=None, model=method)
...
def parse_arguments():
    ...
    parser.add_argument("--data_root", type=str, default="/high_perf_store3/evad-autolabeling/lihao_data/lyp-data/scannet_test_1500", help="Path to the ScanNet dataset.")

    parser.add_argument("--weights", type=str, default="./weights/traqpoint_best.pth", help="Path to the model checkpoint.")
    ...
    parser.add_argument("--detection_threshold", type=float, default=0.0, help="Detection threshold for keypoints.")
...
if __name__ == "__main__":
    ...
    model.detection_threshold = args.detection_threshold
    benchmark = ScanNetPoseMNNBenchmark(data_root=args.data_root)
    model.eval()
    model_helper = traqpoint_helper(model)
    with torch.no_grad():
        method = args.method
        out = benchmark.benchmark(model_helper, model_name='TraqPoint', plot_every_iter=1, plot=args.plot, method=method)
        with open(f'outputs/scannet/traqpoint_{method}.txt', 'w') as f:
            f.write(str(out))